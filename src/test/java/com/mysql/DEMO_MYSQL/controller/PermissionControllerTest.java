...
class PermissionControllerTest {
  ...
  void createPermission_validRequest_success() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            MockMvcResultMatchers.jsonPath("$.message").value("Permission created successfully"))
  ...
  void getAll_success() throws Exception {
    ...
    mockMvc
        .perform(MockMvcRequestBuilders.get("/permissions"))
        .andExpect(MockMvcResultMatchers.status().isOk())
        .andExpect(MockMvcResultMatchers.jsonPath("$.code").value(1000))
        .andExpect(MockMvcResultMatchers.jsonPath("$.success").value(true))
  ...
  void delete_success() throws Exception {
    mockMvc
        .perform(MockMvcRequestBuilders.delete("/permissions/{permissionName}", "USER_READ"))
        .andExpect(MockMvcResultMatchers.status().isOk())
        .andExpect(MockMvcResultMatchers.jsonPath("$.code").value(1000))
        .andExpect(MockMvcResultMatchers.jsonPath("$.success").value(true))
        .andExpect(
            MockMvcResultMatchers.jsonPath("$.message").value("Delete permission successfully"));
